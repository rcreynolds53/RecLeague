...
namespace RecLeagueBlog.Data.Repositories
{
    public class MockPostRepository : IBlogPostRepository
    {
        ...
        {
            return _posts.OrderByDescending(p=>p.DateCreated).ToList();
        }
        ...
        public BlogPost UpdatePostModel(AddPostViewModel postModel)
        {
            ...
            BlogPost convertedPost = _posts.Single(p => p.BlogPostId == postModel.BlogPostId);
            List<Tag> newTags = new List<Tag>();
            foreach (var t in postModel.TagsToPost)
            {
